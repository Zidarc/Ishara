...
# 1. Load the models (auto-select OpenVINO or .pt)
det_weight = WEIGHTS_DIR / "yolo26n_openvino_model"
if not det_weight.exists():
    det_weight = WEIGHTS_DIR / "yolo26n.pt"
if not det_weight.exists():
    det_weight = "yolo26n.pt"

depth_weight = WEIGHTS_DIR / "yolo26s-depth_openvino_model"
if not depth_weight.exists():
    depth_weight = WEIGHTS_DIR / "yolo26s-depth.pt"
if not depth_weight.exists():
    depth_weight = "yolo26s-depth.pt"

det_model = YOLO(str(det_weight)) 
depth_model = YOLO(str(depth_weight))

# 2. Setup video capture and writer
input_video_path = DATASETS_DIR / "IMG_5081.MOV"
if not input_video_path.exists():
    input_video_path = SCRIPT_DIR / "IMG_5081.MOV"
output_video_path = DATASETS_DIR / "IMG_5081_processed.mp4"
