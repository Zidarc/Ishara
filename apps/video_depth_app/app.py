...
def load_models():
    ...
    weights_dir = repo_root / "models" / "base" / "weights"

    # Search in order: OpenVINO directory, local models/, then raw .pt
    det_path = weights_dir / "yolo26n_openvino_model"
    if not det_path.exists():
        det_path = base_dir / "models" / "yolo26n_openvino_model"
    if not det_path.exists():
        det_path = weights_dir / "yolo26n.pt"

    depth_path = weights_dir / "yolo26s-depth_openvino_model"
    if not depth_path.exists():
        depth_path = base_dir / "models" / "yolo26s-depth_openvino_model"
    if not depth_path.exists():
        depth_path = weights_dir / "yolo26s-depth.pt"

    detection_model = YOLO(str(det_path))
    depth_model = YOLO(str(depth_path))
